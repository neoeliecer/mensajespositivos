...
INPUT_FILE = "guion_buda_capitulo_1.md"
OUTPUT_FILE = "guion_buda_capitulo_1.mp3"
...
async def generate_audio():
    ...
    # Extract only the "Guion de Voz en Off" section
    import re
    match = re.search(r"## 🎙️ Guión de Voz en Off\n\n(.*?)(?=\n\n---|\Z)", content, re.DOTALL)
    if match:
        text_to_read = match.group(1).strip()
        # Clean up markers like (Hook), (Cuerpo), [Pausa]
        text_to_read = re.sub(r"\(Hook\)|\(Cuerpo\)|\(Cierre\)", "", text_to_read)
        text_to_read = re.sub(r"\[Pausa.*?\]", "...", text_to_read)
        text_to_read = re.sub(r"\*\*.*?\*\*", lambda m: m.group(0).replace("**", ""), text_to_read) # Remove bold markdown
    else:
