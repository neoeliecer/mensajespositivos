...
def add_text_to_image(input_path, output_path, line1, line2, line3, chapter_text):
    ...
    l3_w, l3_h = get_text_size(line3, title_font)

    max_allowed_w = width * 0.9
    while (l1_w > max_allowed_w or l2_w > max_allowed_w or l3_w > max_allowed_w) and title_font.size > 20:
    ...
    spacing = 20
    total_text_h = chap_h + l1_h + l2_h + l3_h + (spacing * 3)
    center_y = int(height * 0.78) - (total_text_h // 2)

    chap_y = center_y
    l1_y = chap_y + chap_h + spacing
    l2_y = l1_y + l1_h + spacing
    l3_y = l2_y + l2_h + spacing
    ...
    draw_with_shadow(draw, l3_x, l3_y, line3, title_font, gold_color)

    img.save(output_path)
    print(f"Saved to {output_path}")

if __name__ == "__main__":
    search_pattern = r"C:\Users\neo\.gemini\antigravity\brain\fcda4265-34b7-4317-9ae1-fe8e937c9ab2\portada_manos_cap9_base_*.png"
    files = glob.glob(search_pattern)
    if not files:
        print("No input image found.")
    else:
        input_img = max(files, key=os.path.getctime)
        output_img = r"C:\Users\neo\Documents\agente\mensajes positivos\portada_manos_capitulo_9_con_titulo.png"
        add_text_to_image(input_img, output_img, "Psicología", "de los", "Chakras", "Manos que curan - Capítulo 9")
