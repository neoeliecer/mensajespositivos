...
INPUT_FILE = "guion_manos_cap14.md"
OUTPUT_FILE = "guion_manos_cap14.mp3"
...
async def generate_audio():
    ...
    # Extract the script section (ignoring titles and technical details)
    match = re.search(r"## 🎙️ Guión de Voz en Off\n\n(.*?)(?=\n\n---|\Z)", content, re.DOTALL)
    if match:
        text_to_read = match.group(1).strip()
        # Clean tags like (Hook), (Cuerpo), etc.
        text_to_read = re.sub(r"\*\*\(Hook\)\*\*|\*\*\(Cuerpo\)\*\*|\*\*\(Cierre\)\*\*", "", text_to_read)
        text_to_read = re.sub(r"\(Hook\)|\(Cuerpo\)|\(Cierre\)", "", text_to_read)
        # Replace pauses for slightly longer dots if needed
        text_to_read = re.sub(r"\[Pausa.*?\]", "...", text_to_read)
        # Clean extra bold marks
